...
// @route   PUT /api/forums/:id/posts/:postId
// @access  Private
exports.updateForumPost = asyncHandler(async (req, res, next) => {
  let forum = await Forum.findById(req.params.id);
...
// @route   DELETE /api/forums/:id/posts/:postId
// @access  Private
exports.deleteForumPost = asyncHandler(async (req, res, next) => {
  ...
  // Check if user is post owner, teacher, or admin
  const isTeacher = await Course.findOne({
    _id: forum.courseId,
...
// @desc    Toggle like on forum post
